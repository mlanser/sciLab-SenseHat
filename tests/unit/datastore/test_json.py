...
# =========================================================
def test__process_data(sample_data_fields):
    """Happy path! Process data rows."""
    dataHdrs = sample_data_fields
    dataRow = src.utils.datastore.json._process_data([_VALID_DATA_ROW_], dataHdrs['raw'])
    assert len(dataRow[0]) == 5

    dataRow = src.utils.datastore.json._process_data([_TRUNCATED_DATA_ROW_], dataHdrs['raw'])
    assert len(dataRow[0]) == 4
...
def test__process_data_w_bad_params(sample_data_fields):
    ...
    with pytest.raises(AttributeError) as excinfo:
        dataRow = src.utils.datastore.json._process_data('--INVALID--', dataHdrs['raw'])
    exMsg = excinfo.value.args[0]
    assert exMsg == "'str' object has no attribute 'items'"

    with pytest.raises(TypeError) as excinfo:
        dataRow = src.utils.datastore.json._process_data([_VALID_DATA_ROW_], None)
    exMsg = excinfo.value.args[0]
    assert exMsg == "argument of type 'NoneType' is not iterable"

    with pytest.raises(AttributeError) as excinfo:
        dataRow = src.utils.datastore.json._process_data([None], None)
    exMsg = excinfo.value.args[0]
...
def test__read_json_w_bad_params(capsys, binary_data_file, empty_data_file, new_data_file):
    """Test with invalid parameters."""
...
def test__write_json(capsys, new_data_file):
...
def test__write_json_w_bad_params(capsys, new_data_file):
    """Test with invalid parameters."""
    dataFName = new_data_file
    #src.utils.datastore.json._write_json(dataFName, _EVEN_DATA_4xROWS_5xFLDS)
    #data = src.utils.datastore.json._read_json(dataFName)
...
if False:    
  ...
  def test_save_data(faker, sample_data_fields, new_data_file):
      ...
      dataHdrs = sample_data_fields['json']
      dataFName = new_data_file

      src.utils.datastore.json.save_data(dataOut, dataFName, dataHdrs, True)
      ...
      with open(dataFName, 'r', newline='') as dataFile:
          ...
          for i, row in enumerate(dataReader, 0):
              if i < 1:       # Skip first line which holds header names
                  ...
                  dataIn.append(src.utils.datastore.json._process_row(row, dataHdrs))
  ...
  def test_save_data_w_bad_params(capsys, faker, sample_data_fields, empty_data_file, new_data_file):
      ...
      with pytest.raises(TypeError) as excinfo:
          src.utils.datastore.json.save_data(dataOut, None, dataHdrs, False)
      ...
      # Test writing to non-existant file with incorrect 'force' flag
      with pytest.raises(OSError) as excinfo:
          src.utils.datastore.json.save_data(dataOut, dataFName, dataHdrs, False)
  ...
  def test_get_data(capsys, faker, sample_data_fields, new_data_file):
      ...
      with open(dataFName, 'a+', newline='') as dataFile:
          dataWriter = csv.DictWriter(dataFile, _HDR_FLDS_JSON_.keys(), extrasaction='ignore')
          dataWriter.writeheader()
          dataWriter.writerow(_VALID_DATA_ROW_)

      dataIn = src.utils.datastore.json.get_data(dataFName, _HDR_FLDS_RAW_, 1, True)
  ...
  def test_get_data_w_bad_params(capsys, faker, sample_data_fields, new_data_file):
      ...
      exMsg = excinfo.value.args[0]
      assert exMsg == "Data file '_DOES_NOT_EXIST_.JSON.' does not exist!"
      ...
      dataIn = src.utils.datastore.json.get_data(dataFName, _HDR_FLDS_RAW_, 0, True)
      assert len(dataIn[0]) == len(_VALID_DATA_ROW_)

      # ... and try to read too many lines (will read 1 line in this case)
      dataIn = src.utils.datastore.json.get_data(dataFName, _HDR_FLDS_RAW_, 999, True)
      assert len(dataIn[0]) == len(_VALID_DATA_ROW_)
